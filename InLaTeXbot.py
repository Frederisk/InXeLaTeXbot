...
class InLaTeXbot():
    ...
    def __init__(self, updater, devnullChatId=-1):
        ...
        self._latexConverter = LatexConverter(
            self._preambleManager, self._userOptionsManager)
        self._inlineQueryResponseDispatcher = InlineQueryResponseDispatcher(
            updater.bot, self._latexConverter, self._resourceManager, self._userOptionsManager, devnullChatId)
        self._messageQueryResponseDispatcher = MessageQueryResponseDispatcher(
            updater.bot, self._latexConverter, self._resourceManager)
        self._devnullChatId = devnullChatId
        self._messageFilters = []

        self._updater.dispatcher.add_handler(
            CommandHandler('start', self.onStart))
        self._updater.dispatcher.add_handler(
            CommandHandler('abort', self.onAbort))
        self._updater.dispatcher.add_handler(
            CommandHandler("help", self.onHelp))
        self._updater.dispatcher.add_handler(CommandHandler(
            'setcustompreamble', self.onSetCustomPreamble))
        self._updater.dispatcher.add_handler(
            CommandHandler('getmypreamble', self.onGetMyPreamble))
        self._updater.dispatcher.add_handler(CommandHandler(
            'getdefaultpreamble', self.onGetDefaultPreamble))
        self._updater.dispatcher.add_handler(CommandHandler(
            'setcodeincaptionon', self.onSetCodeInCaptionOn))
        self._updater.dispatcher.add_handler(CommandHandler(
            'setcodeincaptionoff', self.onSetCodeInCaptionOff))
        self._updater.dispatcher.add_handler(
            CommandHandler("setdpi", self.onSetDpi))
        self._updater.dispatcher.add_handler(
            MessageHandler(Filters.text, self.dispatchTextMessage), 1)
    ...
    def onStart(self, update, context):
        ...
        update.message.reply_text(
            self._resourceManager.getString("greeting_line_one"))
        with open("resources/demo.png", "rb") as f:
            self._updater.bot.sendPhoto(update.message.from_user.id, f)
        update.message.reply_text(
            self._resourceManager.getString("greeting_line_two"))
    ...
    def onAbort(self, update, context):
        ...
        try:
            ...
            update.message.reply_text(self._resourceManager.getString(
                "preamble_registration_aborted"))
        except KeyError:
            update.message.reply_text(
                self._resourceManager.getString("nothing_to_abort"))
    ...
    def onGetMyPreamble(self, update, context):
        try:
            ...
            update.message.reply_text(self._resourceManager.getString(
                "your_preamble_custom")+preamble)
        except KeyError:
            preamble = self._preambleManager.getDefaultPreamble()
            update.message.reply_text(self._resourceManager.getString(
                "your_preamble_default")+preamble)
    ...
    def onGetDefaultPreamble(self, update, context):
        preamble = self._preambleManager.getDefaultPreamble()
        update.message.reply_text(
            self._resourceManager.getString("default_preamble")+preamble)

        raise DispatcherHandlerStop

    def onSetCustomPreamble(self, update, context):
        self._usersRequestedCustomPreambleRegistration.add(
            update.message.from_user.id)
        update.message.reply_text(
            self._resourceManager.getString("register_preamble"))
    ...
    def onPreambleArrived(self, update, context):
        ...
        if valid:
            ...
            update.message.reply_text(
                self._resourceManager.getString("preamble_registered"))
            self._usersRequestedCustomPreambleRegistration.remove(senderId)
        else:
    ...
    def onExpressionArrived(self, update, context):
        self._messageQueryResponseDispatcher.dispatchMessageQueryResponse(
            update.message)
    ...
    def onSetDpi(self, update, context):
        ...
        try:
            ...
            update.message.reply_text(
                self._resourceManager.getString("dpi_set") % dpi)
        except ValueError:
            update.message.reply_text(
                self._resourceManager.getString("dpi_value_error"))
    ...
    def onInlineQuery(self, update, context):
        ...
        update.inline_query.query = update.inline_query.query.replace(
            "<br/>", "\n")
        self._inlineQueryResponseDispatcher.dispatchInlineQueryResponse(
            update.inline_query)
    ...
    def broadcastHTMLMessage(self, message, userIds, parse_mode="HTML", force=False):
        ...
        send_task = partial(self._sendMessageToUser,
        ...
        for userId in tqdm(self._executor.map(send_task, userIds), total=len(userIds), smoothing=0):
            if userId > 0:
                self.logger.debug(
                    "Broadcast message successfully for user %d" % userId)
            else:
                self.logger.debug(
                    "Failed to broadcast message for user %d" % -userId)

    def _sendMessageToUser(self, userId, message, parse_mode="HTML"):
        attempt = 0
        while attempt < 10:
            try:
                self._updater.bot.sendMessage(
                    userId, message, parse_mode=parse_mode)
                return userId
            except TelegramError as err:
                attempt += 1
                self.logger.warn(
                    "Could not broadcast message for %d, error: %s", userId, str(err))
                sleep(15)
        return -userId
